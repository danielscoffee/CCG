...
int iUpgradeFirstCardByName(PSTRUCT_DECK pstDeck, const char *kpszName, int iDeltaValue, int iDeltaCost)
{
  ...
  for (i = 0; i < pstDeck->iHandCount; i++) {
    if (strcmp(pstDeck->aHand[i].szName, kpszName) == 0) {
      ...
      if (pstDeck->aHand[i].iCost < 0) pstDeck->aHand[i].iCost = 0;
      if (strchr(pstDeck->aHand[i].szName, '+') == NULL) strcat(pstDeck->aHand[i].szName, "+");
      return 1;
    }
  }
  /* draw */
  for (i = 0; i < pstDeck->iDrawCount; i++) {
    if (strcmp(pstDeck->aDraw[i].szName, kpszName) == 0) {
      ...
      if (pstDeck->aDraw[i].iCost < 0) pstDeck->aDraw[i].iCost = 0;
      if (strchr(pstDeck->aDraw[i].szName, '+') == NULL) strcat(pstDeck->aDraw[i].szName, "+");
      return 1;
    }
  }
  /* discard */
  for (i = 0; i < pstDeck->iDiscardCount; i++) {
    if (strcmp(pstDeck->aDiscard[i].szName, kpszName) == 0) {
      ...
      if (pstDeck->aDiscard[i].iCost < 0) pstDeck->aDiscard[i].iCost = 0;
      if (strchr(pstDeck->aDiscard[i].szName, '+') == NULL) strcat(pstDeck->aDiscard[i].szName, "+");
      return 1;
    }
  }
  return 0;
...
//   memcpy(pstDeck->aHand, &stWrkDeck.aHand, pstDeck->iHandCount*(sizeof(STRUCT_CARD)));
//   pstDeck->iHandCount--;
// }
void vShowDeck(PSTRUCT_DECK pstDeck)
{
  ...
  for (i = 0; i < pstDeck->iHandCount; i++)
  {
    ...
    snprintf(szLine, sizeof(szLine), "  [%d]%*.*s%-*.*s (custo:%d, %s:%d)",
             ...
             MAX_HAND, MAX_HAND, pstDeck->aHand[i].szName,
             pstDeck->aHand[i].iCost,

...
void vInitMonsters(PSTRUCT_MONSTER paMonsters, int iCount) {
  int i;
  for (i = 0; i < iCount; i++) {
    snprintf(paMonsters[i].szName, sizeof(paMonsters[i].szName), "Monstro%02d",
             i + 1);
...
void vShowMonsters(PSTRUCT_MONSTER paMonsters, int iCount) {
  ...
  for (i = 0; i < iCount; i++) {
    if (paMonsters[i].iHP > 0) {
      ...
      snprintf(szLine, sizeof(szLine), "  [%d] %s ", i + 1,
               paMonsters[i].szName);
      vPrintColored(szLine, TERMINAL_COLOR_BBLUE);
      vPrintLine(" (", NO_NEW_LINE);
      snprintf(szLine, sizeof(szLine), "HP: %d/%d", paMonsters[i].iHP,
               giMaxMonsterHP);
      vPrintColored(szLine, TERMINAL_COLOR_BRED);

      vPrintLine(" | ", NO_NEW_LINE);
      snprintf(szLine, sizeof(szLine), "BLOCK: %d", paMonsters[i].iBlock);
      vPrintColored(szLine, TERMINAL_COLOR_BCYAN);
      vPrintLine(" | ", NO_NEW_LINE);
      snprintf(szLine, sizeof(szLine), "ATK: %d", paMonsters[i].iAttack);
      vPrintColored(szLine, TERMINAL_COLOR_BMAGENTA);
      if (paMonsters[i].iDebuffCycCt > 0) {
        vPrintLine(" | ", NO_NEW_LINE);
        snprintf(szLine, sizeof(szLine), " Poison/TurnosRestantes=%d/%d",
...
void vDoEnemyActions(PSTRUCT_MONSTER paMonsters, int iMonsterCount) {
  ...
  for (ii = 0; ii < iMonsterCount; ii++) {
    int iChoice;
    if (paMonsters[ii].iHP <= 0)
      continue; /* monstro morto não age */
    if (paMonsters[ii].iDebuffCycCt > 0) {
      ...
      snprintf(szLine, sizeof(szLine),
               ...
               paMonsters[ii].szName, paMonsters[ii].iDebuff,
    ...
    if (iChoice == 0) {
      ...
      snprintf(szLine, sizeof(szLine), "%s ataca causando %d de dano!",
      ...
      snprintf(szLine, sizeof(szLine), "%s se cura em %d (HP=%d).",
...
int iIsMonsterAlive(PSTRUCT_MONSTER pstMonster) { return pstMonster->iHP > 0; }

int iAnyMonsterAlive(PSTRUCT_MONSTER paMonsters, int iCount) {
  int i;
  for (i = 0; i < iCount; i++) {
    if (iIsMonsterAlive(&paMonsters[i]))
      return 1;
  }
  return 0;
}
int iMonsterCountForLevel(int iLevel) {
  ...
  if (iCount > MAX_MONSTERS)
    iCount = MAX_MONSTERS;
  return iCount;
}

void vInitMonstersForLevel(PSTRUCT_MONSTER paMonsters, int iLevel,
                           int *piOutCount) {
  int i;
  int iCount;
  int iAtkBase;
  ...
  *piOutCount = iCount;
  ...
  for (i = 0; i < iCount; i++) {
    snprintf(paMonsters[i].szName, sizeof(paMonsters[i].szName),
             "Monstro %d-%d", iLevel, i + 1);
    giMaxMonsterHP =
        (iLevel - 1) * MONSTER_HP_SCALING_RATE * MONSTER_INITAL_HP_MAX;
